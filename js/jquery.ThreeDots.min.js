...
curr_text_span=$(curr_this).children('.'+$.fn.ThreeDots.c_settings.text_span_class).get(0);var init_text_span=$(curr_text_span).text();if((three_dots_value=$(curr_this).attr('threedots'))!=undefined){$(curr_text_span).text(three_dots_value);$(curr_this).children('.'+$.fn.ThreeDots.c_settings.e_span_class).remove();}
last_text=$(curr_text_span).text();if(last_text.length<=0){last_text='';}
$(curr_this).attr('threedots',last_text);if(num_rows(curr_this)>max_rows){curr_ellipsis=$(curr_this).append('<span style="white-space:nowrap" class="'
+$.fn.ThreeDots.c_settings.e_span_class+'">'
+$.fn.ThreeDots.c_settings.ellipsis_string
+'</span>');while(num_rows(curr_this)>max_rows){lws=the_last_word($(curr_text_span).text());$(curr_text_span).text(lws.updated_string);last_word=lws.word;last_del=lws.del;if(lws.del==null){break;}}
if(last_word!=null){var is_dangling=dangling_ellipsis(curr_this);if((num_rows(curr_this)<=max_rows-1)||(is_dangling)||(!$.fn.ThreeDots.c_settings.whole_word)){last_text=$(curr_text_span).text();if(lws.del!=null){$(curr_text_span).text(last_text+last_del);}
if(num_rows(curr_this)>max_rows){$(curr_text_span).text(last_text);}else{$(curr_text_span).text($(curr_text_span).text()+last_word);if((num_rows(curr_this)>max_rows+1)||(!$.fn.ThreeDots.c_settings.whole_word)||is_dangling){while((num_rows(curr_this)>max_rows)){if($(curr_text_span).text().length>0){$(curr_text_span).text($(curr_text_span).text().substr(0,$(curr_text_span).text().length-1));}else{break;}}}}}}}
...
return $.fn.ThreeDots.the_selected;};$.fn.ThreeDots.settings={valid_delimiters:[' ',',','.'],ellipsis_string:'...',max_rows:2,text_span_class:'ellipsis_text',e_span_class:'threedots_ellipsis',whole_word:true,allow_dangle:false,alt_text_e:false,alt_text_t:false};function dangling_ellipsis(obj){if($.fn.ThreeDots.c_settings.allow_dangle==true){return false;}
var ellipsis_obj=$(obj).children('.'+$.fn.ThreeDots.c_settings.e_span_class).get(0);var remember_display=$(ellipsis_obj).css('display');var num_rows_before=num_rows(obj);$(ellipsis_obj).css('display','none');var num_rows_after=num_rows(obj);$(ellipsis_obj).css('display',remember_display);if(num_rows_before>num_rows_after){return true;}else{return false;}}
function num_rows(obj){var paddingt=parseInt(($(obj).css('padding-top')).replace('px',''));var paddingb=parseInt(($(obj).css('padding-bottom')).replace('px',''));var lineheight=lineheight_px($(obj));var innerh=parseInt($(obj).innerHeight());var n_rows=(innerh-(paddingt+paddingb))/lineheight;return n_rows;}
